...
import me.amitghosh.mathquiz.model.HistoryModel;
import me.amitghosh.mathquiz.model.QuizModel;
import me.amitghosh.mathquiz.utils.Constant;
import me.amitghosh.mathquiz.receiver.NotificationScheduler;
...
public class QuizActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void startTimer(final int count) {
        countDownTimer = new CountDownTimer(count * 1000, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                plusScore = Constant.getPlusScore(countTime);
                tv_plus_score.setText(Constant.getTextString(getString(R.string.addition_sign) + plusScore));
            }

            @Override
            public void onFinish() {
                isTimer = false;
                handler.postDelayed(r, Constant.DELAY_SEOCND);
            }
        }.start();
    }
    ...
    private void init() {
        ...
        tv_set.setText(Constant.getTextString(level + "\n" + getString(R.string.set)));
    ...
    public class GetAllData extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            tv_total_count.setText(Constant.getTextString(getString(R.string.slash) + quizModelList.size()));
    ...
    public void setData(int position) {
        ...
        textView2.setText(Constant.getTextString(quizModel.sign + getString(R.string.single_space) + quizModel.secondDigit));
            ...
            if (main_id == 1) {
                if (isRemider) {
                    if (Integer.parseInt(optionList.get(i)) == Integer.parseInt(quizModel.answer)) {
                        optionViewList.get(i).setText(Constant.getTextString(optionList.get(i) + " " + getString(R.string.rem) + " " + quizModel.rem));
                    } else {
                        optionViewList.get(i).setText(Constant.getTextString(optionList.get(i) + " " + getString(R.string.rem) + " " + (Integer.parseInt(optionList.get(i)) % Integer.parseInt(quizModel.secondDigit))));
                    }
                } else {
                ...
                if (isRemider) {
                    if (Double.parseDouble(optionList.get(i)) == Double.parseDouble(quizModel.answer)) {
                        optionViewList.get(i).setText(Constant.getTextString(optionList.get(i) + " " + getString(R.string.rem) + " " + getFormatValue2(Double.parseDouble(quizModel.rem))));
                    } else {
                        optionViewList.get(i).setText(Constant.getTextString(optionList.get(i) + " " + getString(R.string.rem) + " " + getFormatValue2((Double.parseDouble(optionList.get(i)) % Double.parseDouble(quizModel.secondDigit)))));
                    }
                } else {
                    if (!optionList.get(i).contains(getString(R.string.str_dot))) {
                        ...
                        optionViewList.get(i).setText(Constant.getTextString(optionList.get(i) + getString(R.string.str_dot) + 0));
